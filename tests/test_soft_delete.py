...
class TestSoftDeleteNode:
    ...
    def test_soft_delete_marks_node(self, store):
        ...
        # Create a node
        node = store.write_node(id="test_node_1", label="Test Node", type="concept", content="test content", confidence=0.8, provenance="test")
        ...
        row = store.conn.execute("SELECT id, deleted_at FROM ohm_nodes WHERE id = ?", ["test_node_1"]).fetchone()
    ...
    def test_soft_delete_with_edges(self, store):
        ...
        store.write_node(id="node_a", label="Node A", type="concept", content="a", confidence=0.8, provenance="test")
        store.write_node(id="node_b", label="Node B", type="concept", content="b", confidence=0.8, provenance="test")
        edge = store.write_edge(from_node="node_a", to_node="node_b", edge_type="RELATED_TO", layer="L2", confidence=0.7, provenance="test")
    ...
    def test_double_delete_idempotent(self, store):
        ...
        store.write_node(id="del_twice", label="Delete Twice", type="concept", content="test", confidence=0.5, provenance="test")
    ...
    def test_stats_excludes_soft_deleted(self, store):
        ...
        store.write_node(id="stats_node", label="Stats Node", type="concept", content="test", confidence=0.5, provenance="test")
        # Count nodes before delete
        count_before = store.conn.execute("SELECT COUNT(*) FROM ohm_nodes WHERE deleted_at IS NULL").fetchone()[0]

        store.delete_node("stats_node", deleted_by="test_agent")
        count_after = store.conn.execute("SELECT COUNT(*) FROM ohm_nodes WHERE deleted_at IS NULL").fetchone()[0]
...
class TestSoftDeleteEdge:
    ...
    def test_soft_delete_marks_edge(self, store):
        ...
        store.write_node(id="edge_node_a", label="A", type="concept", content="a", confidence=0.8, provenance="test")
        store.write_node(id="edge_node_b", label="B", type="concept", content="b", confidence=0.8, provenance="test")
        edge = store.write_edge(from_node="edge_node_a", to_node="edge_node_b", edge_type="RELATED_TO", layer="L2", confidence=0.7, provenance="test")
        edge_id = edge["id"]
        ...
        row = store.conn.execute("SELECT id, deleted_at FROM ohm_edges WHERE id = ?", [edge_id]).fetchone()
        assert row is not None
