...
class TestProjectZettelkasten:
    ...
    def test_project_creates_nodes(self, test_db, tmp_path):
        ...
        nodes = test_db.execute(
            "SELECT label, provenance FROM ohm_nodes WHERE provenance = 'metis_zettelkasten'"
        ).fetchall()
        assert len(nodes) == 2
    ...
    def test_project_creates_edges_for_wikilinks(self, test_db, tmp_path):
        ...
        edges = test_db.execute(
            "SELECT edge_type FROM ohm_edges WHERE edge_type = 'REFINES'"
        ).fetchall()
        assert len(edges) == 1
        metis_conn.close()
    ...
    def test_project_is_idempotent(self, test_db, tmp_path):
        ...
        nodes = test_db.execute(
            "SELECT COUNT(*) FROM ohm_nodes WHERE label = 'Idempotent Test'"
        ).fetchone()
        assert nodes[0] == 1
        metis_conn.close()
    ...
    def test_project_records_observation(self, test_db, tmp_path):
        ...
        obs = test_db.execute(
            "SELECT type, value, notes FROM ohm_observations "
            "WHERE type = 'projection'"
        ).fetchall()
        assert len(obs) >= 1
