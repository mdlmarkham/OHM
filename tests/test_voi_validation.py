...
def voi_graph():
    ...
    create_node(conn, label="Root Cause A", node_type="concept", created_by="test", confidence=0.3)
    create_node(conn, label="Root Cause B", node_type="concept", created_by="test", confidence=0.4)
    create_node(conn, label="Root Cause C", node_type="concept", created_by="test", confidence=0.7)
    create_node(conn, label="Root Cause D", node_type="concept", created_by="test", confidence=0.9)
    create_node(conn, label="Root Cause E", node_type="concept", created_by="test", confidence=0.5)

    # Intermediate nodes
    create_node(conn, label="Intermediate X", node_type="concept", created_by="test", confidence=0.5)
    create_node(conn, label="Intermediate Y", node_type="concept", created_by="test", confidence=0.8)

    # Decision nodes (where being wrong matters)
    create_node(conn, label="Decision 1", node_type="decision", created_by="test", confidence=0.6, utility_scale=0.9)
    create_node(conn, label="Decision 2", node_type="decision", created_by="test", confidence=0.7, utility_scale=0.6)

    # Look up generated IDs
    ids = {}
    for label in ["Root Cause A", "Root Cause B", "Root Cause C", "Root Cause D", "Root Cause E", "Intermediate X", "Intermediate Y", "Decision 1", "Decision 2"]:
        row = conn.execute(
    ...
    create_edge(conn, from_node=ids["Root Cause A"], to_node=ids["Intermediate X"], layer="L3", edge_type="CAUSES", created_by="test", confidence=0.8)
    create_edge(conn, from_node=ids["Root Cause B"], to_node=ids["Intermediate X"], layer="L3", edge_type="CAUSES", created_by="test", confidence=0.7)
    create_edge(conn, from_node=ids["Root Cause C"], to_node=ids["Intermediate Y"], layer="L3", edge_type="CAUSES", created_by="test", confidence=0.6)
    create_edge(conn, from_node=ids["Root Cause D"], to_node=ids["Intermediate Y"], layer="L3", edge_type="CAUSES", created_by="test", confidence=0.9)

    # Causal edges: intermediate -> decision
    create_edge(conn, from_node=ids["Intermediate X"], to_node=ids["Decision 1"], layer="L3", edge_type="CAUSES", created_by="test", confidence=0.8)
    create_edge(conn, from_node=ids["Intermediate Y"], to_node=ids["Decision 1"], layer="L3", edge_type="CAUSES", created_by="test", confidence=0.7)
    create_edge(conn, from_node=ids["Intermediate X"], to_node=ids["Decision 2"], layer="L3", edge_type="CAUSES", created_by="test", confidence=0.6)
    create_edge(conn, from_node=ids["Intermediate Y"], to_node=ids["Decision 2"], layer="L3", edge_type="CAUSES", created_by="test", confidence=0.5)

    # Direct influence (L3 doesn't have INFLUENCES, use CAUSES)
    create_edge(conn, from_node=ids["Root Cause E"], to_node=ids["Decision 1"], layer="L3", edge_type="CAUSES", created_by="test", confidence=0.4)
...
class TestVoIValidation:
    ...
    def test_voi_identifies_uncertain_roots(self, voi_graph):
        ...
        if root_a_id in rankings and root_d_id in rankings:
            assert rankings[root_a_id]["voi_score"] >= rankings[root_d_id]["voi_score"], f"Root Cause A (uncertain) should have >= VoI than Root Cause D (certain), got {rankings[root_a_id]['voi_score']:.4f} vs {rankings[root_d_id]['voi_score']:.4f}"
    ...
    def test_voi_scores_are_positive(self, voi_graph):
        ...
        for ranking in result["rankings"]:
            assert ranking["voi_score"] >= 0, f"VoI score for {ranking['node_id']} should be non-negative, got {ranking['voi_score']}"
    ...
    def test_voi_uncertainty_correlates_with_low_confidence(self, voi_graph):
        ...
        if root_a_id in rankings and root_d_id in rankings:
            assert rankings[root_a_id]["uncertainty"] >= rankings[root_d_id]["uncertainty"], "Lower confidence should mean higher uncertainty"
    ...
    def test_high_voi_observations_improve_confidence(self, voi_graph):
        ...
        initial_decisions = conn.execute("SELECT id, confidence FROM ohm_nodes WHERE type = 'decision' AND deleted_at IS NULL").fetchall()
        ...
        for node_id in high_voi_nodes:
            create_observation(conn=conn, node_id=node_id, obs_type="measurement", value=0.9, created_by="test_agent", notes="High-VoI validation observation")

        # Step 4: Recompute confidence on decision nodes
        # After observations, the Bayesian update should increase confidence
        updated_decisions = conn.execute("SELECT id, confidence FROM ohm_nodes WHERE type = 'decision' AND deleted_at IS NULL").fetchall()
    ...
    def test_low_voi_nodes_have_lower_impact(self, voi_graph):
        ...
        top = rankings[0]
        bottom = rankings[-1]
        assert top["voi_score"] >= bottom["voi_score"], f"Top VoI ({top['voi_score']:.4f}) should be >= bottom ({bottom['voi_score']:.4f})"
...
class TestVoIDependsOnInclusion:
    ...
    def depends_graph(self):
        ...
        # Create nodes
        create_node(conn, label="Root A", node_type="concept", created_by="test", confidence=0.3)
        create_node(conn, label="Root B", node_type="concept", created_by="test", confidence=0.4)
        create_node(conn, label="Root C", node_type="concept", created_by="test", confidence=0.5)
        create_node(conn, label="Decision 1", node_type="decision", created_by="test", confidence=0.6, utility_scale=0.9)
        create_node(conn, label="Decision 2", node_type="decision", created_by="test", confidence=0.7, utility_scale=0.6)
        ...
        # CAUSES edge (L3)
        create_edge(conn, from_node=ids["Root A"], to_node=ids["Decision 1"], layer="L3", edge_type="CAUSES", created_by="test", confidence=0.8)
        # DEPENDS_ON edge (L4) — the key edge type we're testing
        create_edge(conn, from_node=ids["Root B"], to_node=ids["Decision 1"], layer="L4", edge_type="DEPENDS_ON", created_by="test", confidence=0.7)
        # Another CAUSES edge for comparison
        create_edge(conn, from_node=ids["Root C"], to_node=ids["Decision 2"], layer="L3", edge_type="CAUSES", created_by="test", confidence=0.6)
    ...
    def test_depends_on_included_by_default(self, depends_graph):
        ...
        assert ids["Root B"] in ranked_ids, "Root B (connected via DEPENDS_ON) should appear in VoI rankings by default"
    ...
    def test_depends_on_excluded_when_filtered(self, depends_graph):
        ...
        assert ids["Root B"] not in ranked_ids, "Root B should NOT appear when DEPENDS_ON is excluded from edge_types"
    ...
    def test_depends_on_in_default_edge_types(self):
        ...
        """
        conn = create_test_db()
        create_node(conn, label="Prereq", node_type="concept", created_by="test", confidence=0.3)
        create_node(conn, label="Action", node_type="decision", created_by="test", confidence=0.6, utility_scale=0.8)
        ids = {}
        for label in ["Prereq", "Action"]:
            ...
            ids[label] = row[0]
        create_edge(conn, from_node=ids["Prereq"], to_node=ids["Action"], layer="L4", edge_type="DEPENDS_ON", created_by="test", confidence=0.7)
        # Call with default (None) — should include DEPENDS_ON
        result = compute_voi(conn)
        ranked_ids = {r["node_id"] for r in result["rankings"]}
        assert ids["Prereq"] in ranked_ids, "DEPENDS_ON edges should be included by default in compute_voi"
        conn.close()
    ...
    def test_voi_with_only_depends_on(self, depends_graph):
        ...
        assert ids["Root B"] in ranked_ids, "Root B should appear when only DEPENDS_ON edges are considered"
        # Root A and Root C only have CAUSES edges — should NOT appear
        assert ids["Root A"] not in ranked_ids, "Root A (CAUSES only) should NOT appear when only DEPENDS_ON is considered"
        assert ids["Root C"] not in ranked_ids, "Root C (CAUSES only) should NOT appear when only DEPENDS_ON is considered"
...
class TestVoIAdjacencyDirection:
    ...
    def chain_graph(self):
        ...
        create_node(conn, label="Root", node_type="concept", created_by="test", confidence=0.3)
        create_node(conn, label="Mid1", node_type="concept", created_by="test", confidence=0.5)
        create_node(conn, label="Mid2", node_type="concept", created_by="test", confidence=0.6)
        create_node(conn, label="Decision", node_type="decision", created_by="test", confidence=0.7, utility_scale=0.9)
        # Unrelated node — not an ancestor of Decision
        create_node(conn, label="Unrelated", node_type="concept", created_by="test", confidence=0.4)
        ...
        create_edge(conn, from_node=ids["Root"], to_node=ids["Mid1"], layer="L3", edge_type="CAUSES", created_by="test", confidence=0.8)
        create_edge(conn, from_node=ids["Root"], to_node=ids["Mid2"], layer="L3", edge_type="CAUSES", created_by="test", confidence=0.7)
        # Mid1 and Mid2 CAUSES Decision (forward: Mid1 → Decision, Mid2 → Decision)
        create_edge(conn, from_node=ids["Mid1"], to_node=ids["Decision"], layer="L3", edge_type="CAUSES", created_by="test", confidence=0.8)
        create_edge(conn, from_node=ids["Mid2"], to_node=ids["Decision"], layer="L3", edge_type="CAUSES", created_by="test", confidence=0.6)
    ...
    def test_ancestors_found_correctly(self, chain_graph):
        ...
        assert ids["Root"] in ranked_ids, "Root should be in VoI rankings (ancestor of Decision)"
        assert ids["Mid1"] in ranked_ids, "Mid1 should be in VoI rankings (ancestor of Decision)"
        assert ids["Mid2"] in ranked_ids, "Mid2 should be in VoI rankings (ancestor of Decision)"

        # Unrelated is NOT an ancestor of Decision
        assert ids["Unrelated"] not in ranked_ids, "Unrelated should NOT be in VoI rankings (not an ancestor of Decision)"
    ...
    def test_decision_not_in_own_ancestors(self, chain_graph):
        ...
        ranked_ids = {r["node_id"] for r in result["rankings"]}
        assert ids["Decision"] not in ranked_ids, "Decision node should not appear in its own VoI rankings"
    ...
    def test_path_confidence_direction(self, chain_graph):
        ...
        assert ids["Decision"] in root_ranking["downstream_decisions"], "Root's downstream_decisions should include Decision"
...
class TestVoISensitivityATE:
    ...
    def ate_graph(self):
        ...
        create_node(conn, label="Root", node_type="concept", created_by="test", confidence=0.3)
        create_node(conn, label="Decision", node_type="decision", created_by="test", confidence=0.6, utility_scale=0.9)
        ...
        create_edge(conn, from_node=ids["Root"], to_node=ids["Decision"], layer="L3", edge_type="CAUSES", created_by="test", confidence=0.8)
    ...
    def test_sensitivity_method_field_exists(self, ate_graph):
        ...
        result = compute_voi(conn)
        for ranking in result["rankings"]:
            assert "sensitivity_method" in ranking, f"Missing sensitivity_method in ranking for {ranking['node_id']}"
    ...
    def test_sensitivity_method_is_ate_when_available(self, ate_graph):
        ...
        assert "ate" in ate_methods, f"Expected 'ate' in sensitivity methods when pgmpy available, got {ate_methods}"
    ...
    def test_sensitivity_method_fallback(self, ate_graph):
        ...
        for ranking in result["rankings"]:
            assert ranking["sensitivity_method"] in valid_methods, f"Invalid sensitivity_method: {ranking['sensitivity_method']}"
    ...
    def test_ate_sensitivity_differs_from_path_confidence(self, ate_graph):
        ...
        for ranking in result["rankings"]:
            assert ranking["sensitivity"] >= 0, f"Sensitivity should be non-negative, got {ranking['sensitivity']}"
