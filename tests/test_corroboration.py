...
def _seed_edge(conn, from_node: str, to_node: str, created_by: str,
               edge_type: str = "CAUSES", layer: str = "L3",
...
class TestComputeEdgeCorroboration:
    ...
    def test_single_edge_has_zero_corroboration(self):
        ...
        try:
            ...
            count = conn.execute(
                "SELECT corroboration_count FROM ohm_edges WHERE from_node = 'src1'"
            ).fetchone()[0]
            assert count == 0
        finally:
            conn.close()
    ...
    def test_two_agents_same_claim_corroborate(self):
        ...
        try:
            ...
            counts = conn.execute(
                "SELECT corroboration_count FROM ohm_edges WHERE to_node = 'target' ORDER BY created_by"
            ).fetchall()
    ...
    def test_three_agents_give_count_2(self):
        ...
        try:
            ...
            counts = conn.execute(
                "SELECT corroboration_count FROM ohm_edges WHERE to_node = 'target'"
            ).fetchall()
    ...
    def test_same_agent_multiple_edges_no_corroboration(self):
        ...
        try:
            ...
            assert all(r[0] == 0 for r in counts), (
                "Same agent making multiple edges should NOT corroborate"
            )
        finally:
            conn.close()
    ...
    def test_soft_deleted_edges_excluded(self):
        ...
        try:
            ...
            count_b = conn.execute(
                "SELECT corroboration_count FROM ohm_edges WHERE created_by = 'agent_b'"
            ).fetchone()[0]
            assert count_b == 0
        finally:
            conn.close()
    ...
    def test_fresh_db_has_column(self):
        ...
        try:
            cols = conn.execute(
                ...
                "WHERE table_name = 'ohm_edges' AND column_name = 'corroboration_count'"
            ).fetchall()
            assert len(cols) == 1
        finally:
            conn.close()
