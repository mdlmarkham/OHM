...
def _seed_node(conn, node_id: str, provenance: str | None = None, created_by: str = "seeder") -> None:
    if provenance:
        conn.execute(
            "INSERT INTO ohm_nodes (id, label, type, created_by, provenance, created_at) "
...
def _seed_edge(conn, from_node: str, to_node: str, created_by: str) -> None:
    ...
    conn.execute(
        ...
        "VALUES (?, ?, ?, 'L3', 'CAUSES', 0.7, ?, CURRENT_TIMESTAMP)",
...
class TestDomainAutoPopulation:
    ...
    def test_domain_set_from_node_provenance(self):
        ...
        try:
            ...
            result = query_record_outcome(
                conn, source_agent="agent_b", claim_node="cattle_claim",
    ...
    def test_domain_defaults_to_star_when_no_provenance(self):
        ...
        try:
            ...
            result = query_record_outcome(
                conn, source_agent="agent_b", claim_node="no_prov_claim",
                outcome=True, recorded_by="agent_b",
            )
...
class TestDomainAwareReliability:
    ...
    def test_domain_filter_counts_only_matching(self):
        ...
        try:
            ...
            query_record_outcome(conn, source_agent="agent_b", claim_node="c1",
                                  outcome=True, recorded_by="agent_b")
            ...
            query_record_outcome(conn, source_agent="agent_b", claim_node="c2",
                                  outcome=True, recorded_by="agent_b")
            ...
            query_record_outcome(conn, source_agent="agent_b", claim_node="f1",
                                  outcome=False, recorded_by="agent_b")
    ...
    def test_unscoped_outcomes_count_in_all_domains(self):
        ...
        try:
            ...
            query_record_outcome(conn, source_agent="agent_b",
                                  claim_node="unscoped_claim",
                                  outcome=True, recorded_by="agent_b")

            # Filtering by any domain should still find this outcome
            rel = query_source_reliability(conn, "agent_a", domain="cattle-health")
            assert rel["total_outcomes"] == 1, (
                "Unscoped outcomes should count in all domain filters"
            )
        finally:
            conn.close()
...
class TestSchemaMigration:
    ...
    def test_fresh_db_has_domain_column(self):
        ...
        try:
            cols = conn.execute(
                ...
                "WHERE table_name = 'ohm_outcomes' AND column_name = 'domain'"
            ).fetchall()
            assert len(cols) == 1
        finally:
            conn.close()
    ...
    def test_backfill_from_node_provenance(self):
        ...
        try:
            ...
            import uuid
            conn.execute(
                "INSERT INTO ohm_outcomes (id, source_agent, claim_node, outcome, recorded_by) "
                "VALUES (?, 'agent', 'old_claim', TRUE, 'agent')",
                [str(uuid.uuid4())],
            )
            # Run backfill
            conn.execute(
            ...
            row = conn.execute(
                "SELECT domain FROM ohm_outcomes WHERE claim_node = 'old_claim'"
            ).fetchone()
