...
def create_node(base: str, headers: dict, node_id: str, label: str, node_type: str, content: str, confidence: float, provenance: str) -> None:
    r = requests.post(
...
def create_edge(base: str, headers: dict, from_id: str, to_id: str, edge_type: str, layer: str, confidence: float, provenance: str, probability: float | None = None) -> None:
    body = {
        "from": from_id,
        "to": to_id,
        "type": edge_type,
...
def seed_cluster(base: str, headers: dict) -> None:
    ...
    create_node(
        base,
        headers,
        "auth-accountability-gap",
        "Authorization Accountability Gap",
        "vulnerability",
    ...
    create_node(
        base,
        headers,
        "gov-devenex-execution",
        "Devenex Agent Execution Model",
        "concept",
    ...
    create_edge(base, headers, "adv-prompt-injection", "auth-scope-gap", "THREATENS", "L4", 0.90, "metis-security", 0.85)
    create_edge(base, headers, "adv-prompt-injection", "auth-control-gap", "THREATENS", "L4", 0.88, "metis-security", 0.80)
    create_edge(base, headers, "adv-nl-coercion", "auth-visibility-gap", "THREATENS", "L4", 0.85, "metis-security", 0.75)
    create_edge(base, headers, "adv-nl-coercion", "auth-scope-gap", "THREATENS", "L4", 0.82, "metis-security", 0.70)
    create_edge(base, headers, "adv-mid-execution-evasion", "auth-control-gap", "THREATENS", "L4", 0.88, "metis-security", 0.82)
    create_edge(base, headers, "adv-mid-execution-evasion", "auth-accountability-gap", "THREATENS", "L4", 0.85, "metis-security", 0.78)
    create_edge(base, headers, "adv-identity-theft", "auth-accountability-gap", "THREATENS", "L4", 0.92, "metis-security", 0.88)
    create_edge(base, headers, "adv-identity-theft", "auth-visibility-gap", "THREATENS", "L4", 0.80, "metis-security", 0.72)

    # ── Edges: Attack patterns cause each other (attack chains) ──────

    print("Creating attack chain edges...")
    create_edge(base, headers, "adv-prompt-injection", "adv-mid-execution-evasion", "CAUSES", "L3", 0.75, "metis-security", 0.65)
    create_edge(base, headers, "adv-nl-coercion", "adv-identity-theft", "CAUSES", "L3", 0.70, "metis-security", 0.60)
    create_edge(base, headers, "adv-mid-execution-evasion", "adv-identity-theft", "CAUSES", "L3", 0.65, "metis-security", 0.55)

    # ── Edges: Governance mitigates authorization gaps ───────────────

    print("Creating governance→vulnerability edges...")
    create_edge(base, headers, "gov-telos-governance", "auth-scope-gap", "DEPENDS_ON", "L4", 0.80, "metis-security")
    create_edge(base, headers, "gov-telos-governance", "auth-accountability-gap", "DEPENDS_ON", "L4", 0.78, "metis-security")
    create_edge(base, headers, "gov-devenex-execution", "auth-control-gap", "DEPENDS_ON", "L4", 0.82, "metis-security")
    create_edge(base, headers, "gov-devenex-execution", "auth-visibility-gap", "DEPENDS_ON", "L4", 0.75, "metis-security")

    # ── Edges: Link to existing AND→OR framework ────────────────────

    print("Linking to AND→OR framework...")
    create_edge(base, headers, "adv-prompt-injection", "concept-agent-authorization-gap", "APPLIES_TO", "L3", 0.90, "metis-security")
    create_edge(base, headers, "adv-mid-execution-evasion", "concept-least-privilege-as-and-gate", "APPLIES_TO", "L3", 0.85, "metis-security")
    create_edge(base, headers, "adv-identity-theft", "concept-agent-identity-stability", "APPLIES_TO", "L3", 0.92, "metis-security")
    create_edge(base, headers, "auth-scope-gap", "concept-privilege-escalation-path", "APPLIES_TO", "L3", 0.88, "metis-security")
    create_edge(base, headers, "auth-accountability-gap", "concept-delegated-authority-problem", "APPLIES_TO", "L3", 0.85, "metis-security")
...
def main():
    ...
    parser.add_argument("--base", default="http://127.0.0.1:8710", help="OHM daemon base URL")
    parser.add_argument("--token", default=None, help="Bearer token (or set OHM_TOKEN env var)")
