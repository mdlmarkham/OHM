...
```
 ...
 │ agents/metis/│    │  + Parquet data              │    │ agents/clio/  │
...
**Five decay profiles** (ADR-014):

| Profile | Half-life | Use case |
|---------|-----------|----------|
| `perishable` | 7 days | Prices, sentiment, breaking events |
| `fast_perishable` | 3 days | Ephemeral signals, rumors |
| `durable` | 180 days | Structural facts, geography, law |
| `binary` | ∞ (no decay) | Mathematical truths, definitions |
| `appreciating` | −30 days (improves with age) | Reputation, verified patterns |
...
GET  /confidence_audit?node_id=concept-x   # shows decay curve
```
...
GET /suggest?method=nudge&agent=metis
```
...
Source reliability tracking: `graph.source_reliability("agent-clio")` returns accuracy rates based on recorded outcomes.
...
Current challenge ratio: **6.1%** (target: ≥5%). When agents don't challenge enough, the nudge system surfaces unchallenged high-confidence edges.
...
GET  /suggest?method=nudge&agent=metis          # Proactive discoverability
...
GET  /source_reliability?source=agent-clio      # Source accuracy tracking
...
# Tasks
GET  /tasks?status=open&assigned_to=metis      # Query tasks
...
**OHM v0.25.0.** Production use by 11 agents (Atlas, Métis, Clio, Socrates, Hephaestus, Deepthought, Hera, Stheno, Prometheus, Epimetheus, Persephone).

**Graph**: 1,621 nodes · 2,634 edges · 1,146 observations · 3,139 aliases · 530 content hashes · 6.1% challenge ratio
...
|---------|-----------|-------------|
| Temporal decay | OHM-xdd4, OHM-wuki | Five decay profiles, chain validity (STL weakest-link), `confidence_at()` |
...
|----------|-------|-------------|
| P0 | [a5rz](docs/adr/ADR-019-l0-thinking-layer.md) | L0 thinking layer (ADR written, implementation next) |
| P2 | 24g9 | Weibull decay profiles (Phase 4, after Phase 3 validation) |
| P2 | 6lvk | Graph health scoring and remediation |
| P2 | od01.4 | Causal discovery from observation data |
| P2 | wdrg.3 | Normalize free-text source fields to source nodes |
| P3 | tr71.7 | Challenge ratio nudge for low-challenge agents |
| P3 | od01.5 | VoI→action: decision-theoretic edge creation |
