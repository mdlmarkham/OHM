...
NODE_MAP = {
    'id': 'id', 'label': 'label', 'type': 'type', 'content': 'content',
    'url': 'url', 'created_by': 'created_by', 'created_at': 'created_at',
    'updated_at': 'updated_at', 'updated_by': 'updated_by',
    'confidence': 'confidence', 'visibility': 'visibility',
    'provenance': 'provenance', 'tags': 'tags', 'metadata': 'metadata',
    'priority': 'priority',
...
EDGE_MAP = {
    ...
    'edge_type': 'edge_type', 'layer': 'layer', 'confidence': 'confidence',
    'condition': 'condition', 'probability': 'probability',
    'urgency': 'urgency', 'challenge_of': 'challenge_of',
...
OBS_MAP = {
    'id': 'id', 'node_id': 'node_id', 'edge_id': 'edge_id', 'type': 'type',
    'value': 'value', 'baseline': 'baseline', 'sigma': 'sigma',
    'source': 'source', 'created_by': 'created_by',
    'created_at': 'created_at', 'metadata': 'metadata', 'notes': 'notes',
    'source_name': 'source_name', 'source_url': 'source_url',
    # sentiment not in DuckLake mirror - will be NULL
}

def rebuild():
    print(f"Rebuilding from DuckLake mirror...")
            ...
            try:
                ...
                for i, (dl, local) in enumerate(NODE_MAP.items()):
                    if local == 'confidence' and values[i] is not None:
                        ...
                        try: values[i] = str(values[i])  # Let DuckDB handle the cast
                        except: pass
