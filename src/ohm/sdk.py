...
class Graph:
    ...
    def status(self) -> dict[str, Any]:
        """Graph status with schema version — same as CLI 'ohm graph status'.

        Returns stats plus schema_version for agent introspection.
        ADR-005: Self-documenting interface for agents.
        """
        from ohm.queries import query_stats
        from ohm.schema import get_schema_version

        result = query_stats(self._conn)
        result["schema_version"] = get_schema_version(self._conn)
        return result
    ...
    def query(
        ...
        *,
        ...
        """Structured or freeform graph query — same as CLI 'ohm graph query'.

        If *text* is provided, searches nodes by label/content match.
        Otherwise, filters edges by type, layer, owner, and/or confidence.

        Args:
            text: Freeform text search on node labels and content.
            edge_type: Filter edges by type (e.g., 'CAUSES').
            layer: Filter edges by layer (e.g., 'L3').
            owner: Filter edges by creating agent.
            confidence_min: Minimum confidence threshold.
        ...
        if text:
            result = self._conn.execute(
                "SELECT * FROM ohm_nodes WHERE label ILIKE ? OR content ILIKE ? LIMIT ?",
                [f"%{text}%", f"%{text}%", limit],
            )
        elif edge_type or layer or owner or confidence_min is not None:
            conditions = []
            params: list[Any] = []
            if edge_type:
                conditions.append("edge_type = ?")
    ...
    def apply_decay(
        ...
        *,
        ...
        """Apply confidence decay to stale edges — same as CLI 'ohm graph decay'.
        ...
        Returns:
            Dict with 'updated' (int), 'skipped' (int), and 'decayed' (list).
        """
    ...
    def upgrade(self, *, dry_run: bool = False) -> dict[str, Any]:
        """Apply pending schema migrations — same as CLI 'ohm graph upgrade'.

        Checks the current schema version against available migrations
        and applies any that are pending.

        Args:
            dry_run: If True, report pending migrations without applying them.

        Returns:
            Dict with 'current_version', 'target_version', 'pending' (list),
            and 'applied' (int, 0 if dry_run).
        """
        from ohm.schema import SCHEMA_VERSION, MIGRATIONS, get_schema_version

        current = get_schema_version(self._conn)
        pending = []
        for version, description, _statements in MIGRATIONS:
            if current < version:
        ...
        if not dry_run and pending:
            ...
            result["current_version"] = get_schema_version(self._conn)
