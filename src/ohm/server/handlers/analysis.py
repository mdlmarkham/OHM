...
class AnalysisHandlerMixin:
    ...
    # ── Causal Discovery (OHM-od01.4) ──

    def _get_discover(self, path: str, qs: dict) -> None:
        """GET /discover — Causal discovery from observation data.

        Query params:
            nodes: Comma-separated node IDs (required)
            method: pc, ges, correlation, auto (default: auto)
            alpha: Significance level for PC (default: 0.05)
            min_obs: Minimum observations per node (default: 3)
        """
        from ohm.exceptions import ValidationError
        from ohm.graph.discovery import run_discovery, build_observation_matrix

        nodes_param = qs.get("nodes", [None])[0]
        if not nodes_param:
            raise ValidationError("?nodes=node1,node2,... is required")

        node_ids = [n.strip() for n in nodes_param.split(",") if n.strip()]
        if len(node_ids) < 2:
            raise ValidationError("At least 2 node IDs required for discovery")

        method = qs.get("method", ["auto"])[0]
        if method not in ("pc", "ges", "correlation", "auto"):
            raise ValidationError(f"Invalid method: {method}. Use pc, ges, correlation, or auto")
        ...
        try:
            min_obs = int(qs.get("min_obs", ["3"])[0])
        except (ValueError, TypeError):
            min_obs = 3

        # Fetch observations for requested nodes
        conn = self.current_store.read_conn
        placeholders = ",".join(["?"] * len(node_ids))
        rows = conn.execute(
            f"""SELECT node_id, value, created_at, type, source
        ...
        observations = [
            {"node_id": r[0], "value": r[1], "created_at": r[2], "type": r[3], "source": r[4]}
            for r in rows
        ]
        ...
        if len(observations) < 5:
            self._json_response(200, {
                ...
                "warnings": [f"Only {len(observations)} observations found — need at least 5 for discovery"],
                "total_candidates": 0,
            })
            return
        ...
        # Store candidates in discovery queue for agent review
        conn = self.current_store.conn
        queued = 0
            ...
            try:
                conn.execute(
                    """INSERT INTO ohm_discovery_queue
                       (from_node, to_node, method, confidence, edge_type, status, p_value, metadata)
                       VALUES (?, ?, ?, ?, ?, 'pending', ?, ?)""",
                    [
                        candidate.from_node,
                        candidate.to_node,
                ...
                logger.warning(f"Failed to queue discovery candidate {candidate.from_node} → {candidate.to_node}: {e}")

        result.warnings.append(f"{queued} candidates queued for agent review")

        self._json_response(200, result.to_dict())

    def _get_discovery_queue(self, path: str, qs: dict) -> None:
        """GET /discover/queue — Pending discovery candidates for agent review."""
        # Discovery queue is stored in a simple table
        conn = self.current_store.read_conn
        ...
        try:
            rows = conn.execute(
                """SELECT id, from_node, to_node, method, confidence, p_value, edge_type,
                          metadata, status, created_at, reviewed_by, reviewed_at, review_notes
    ...
    def _post_discovery_review(self, path: str, qs: dict, body: dict) -> None:
        """POST /discover/queue/review — Accept or reject a discovery candidate.

        Body: {"candidate_id": "...", "action": "accept"|"reject", "notes": "...",
               "edge_type": "CAUSES"|"SUPPORTS"|..., "confidence": 0.8}
        """
        self._require_write_auth()
        ...
        if not candidate_id or action not in ("accept", "reject"):
            raise ValidationError("candidate_id and action (accept/reject) required")

        conn = self.current_store.conn

        # Get the candidate
        row = conn.execute(
            "SELECT from_node, to_node, method, confidence, edge_type FROM ohm_discovery_queue WHERE id = ?",
            [candidate_id],
        ).fetchone()
        ...
        from_node, to_node, method, disc_confidence, suggested_type = row

        agent = self._get_agent()
        notes = body.get("notes", "")

        if action == "accept":
            # Add as L2 edge with provenance=structure_learning
            edge_type = body.get("edge_type", suggested_type.replace("SUGGESTED_", ""))
            confidence = body.get("confidence", disc_confidence)
            ...
            self.current_store.write_edge(
                ...
                notes=f"Discovered via {method}. {notes}".strip(),
            )

            conn.execute(
                "UPDATE ohm_discovery_queue SET status = ?, reviewed_by = ?, reviewed_at = ?, review_notes = ? WHERE id = ?",
                ["accepted", agent, self.current_store._now(), notes, candidate_id],
            )
            ...
            conn.execute(
                "UPDATE ohm_discovery_queue SET status = ?, reviewed_by = ?, reviewed_at = ?, review_notes = ? WHERE id = ?",
                ["rejected", agent, self.current_store._now(), notes, candidate_id],
            )
