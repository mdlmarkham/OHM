        ...
        self._json_response(
            200,
            {
                ...
                "endpoints": {
                    ...
                    "/plans": {"method": "GET", "description": "List TOPO plans with filters (?node_id=, ?plan_type=, ?status=, ?horizon=)"},
                    "/reports": {"method": "GET", "description": "List TOPO reports with filters (?type=, ?node_id=, ?plan_id=, ?status=)"},
                    "/runs": {"method": "GET", "description": "List TOPO runs with filters (?report_id=, ?node_id=, ?type=, ?status=)"},
                    "/rul": {"method": "GET", "description": "List RUL assessments with filters (?equipment_id=, ?risk_class=, ?site_id=)"},
                    "/timeline/{id}": {"method": "GET", "description": "Roll up events from CONTAINS subtree (?horizon=, ?start_after=, ?end_before=)"},
                    "/report/{id}": {"method": "GET", "description": "Fetch a single TOPO report"},
                    "/run/{id}": {"method": "GET", "description": "Fetch a single TOPO run"},
            ...
            try:
                ...
                try:
                    ...
                    if last_sync_at is not None:
                        ...
                        lines += [
                            "# HELP ohm_ducklake_sync_lag_seconds Seconds since last DuckLake sync",
                            "# TYPE ohm_ducklake_sync_lag_seconds gauge",
                            f"ohm_ducklake_sync_lag_seconds {lag_seconds}",
                            "# HELP ohm_ducklake_last_sync_timestamp Unix timestamp of last DuckLake sync",
                            "# TYPE ohm_ducklake_last_sync_timestamp gauge",
