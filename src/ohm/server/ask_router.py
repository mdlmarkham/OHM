...
INTENT_KEYWORDS: dict[str, list[str]] = {
    "analytics": [
        "metrics", "analytics", "kpi", "dashboard", "measure", "measurement",
        "statistic", "stats", "semantic-layer metrics", "semantic metrics",
        "metric", "how many", "count of", "trend", "health score",
    ],
    "explore": [
        "explore", "search", "find", "lookup", "look up", "neighborhood", "around",
        "connected to", "related to", "what is linked", "what relates to", "graph",
        "show me", "where is", "path from", "path to", "similar to", "semantic",
    ],
    "challenge": [
        "challenge", "contradict", "disagree", "objection", "refute", "rebuttal",
        "flaw", "what is wrong", "why is this wrong", "critique", "criticism",
    ],
    "what-if": [
        "what if", "what-if", "scenario", "simulate", "game theory", "nash",
        "policy", "pomdp", "belief", "decision", "best action", "expected value",
        "intervene", "inference", "would happen", "predict", "what happens",
    ],
    "help": [
        "help", "openapi", "api docs", "documentation", "what can you do",
        "endpoints", "how do i", "how to use", "schema", "layers",
    ],
}
...
def _extract_node_id(text: str) -> str | None:
    ...
    if not text:
        return None
    stop_words = {
        "what", "if", "the", "and", "edge", "node", "for", "how", "are", "this",
        "that", "with", "from", "causes", "fails", "under", "pressure", "around",
        "explore", "search", "find", "lookup", "look", "up", "neighborhood",
        "challenge", "metrics", "semantic", "what-if", "whatif",
    }
