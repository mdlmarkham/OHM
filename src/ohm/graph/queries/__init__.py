...
def register_rul_assessment(
    ...
    *,
    ...
    Creates a row in ``topo_prospects`` for the assessment, then creates an
    L4 ``PREDICTS`` edge from the equipment node to a RUL summary node so the
    prediction is reachable from the graph. If the equipment node does not
    exist, only the prospect row is stored (the edge is skipped).

    Args:
        conn: Active DuckDB connection.
        equipment_node_id: OHM node id for the equipment being assessed.
        rul_days: Remaining useful life in days.
        risk_class: Risk classification (e.g., 'critical', 'high', 'medium', 'low').
        model_version: Optional model version string (e.g., 'weibull_2p_v3').
        site_id: Optional site identifier.
        node_path: Optional UNS address path (OHM-ivlt). Stored in metadata.
        metadata: Optional extensible metadata dict.
        created_by: Agent creating the assessment.

    Returns:
        Dict with the prospect row and the edge id (or None if no edge was created).
    """
    import json
    import uuid
...
def set_node_path(
    ...
    *,
    ...
    Args:
        ...
        created_by: Agent performing the update.
...
def get_nodes_by_path_prefix(
    ...
    *,
    ...
    Enables hierarchical queries like "all nodes under pns.fm10" by
    matching path prefix 'pns.fm10'.
