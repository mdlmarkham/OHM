...
# Postgres
# ---------------------------------------------------------------------------
def writeback_to_postgres(*, duckdb_path, source_sql, host, port, dbname,
                           user, password, pg_schema, pg_table, conflict_key, dry_run):
    import duckdb
    h = _load_helpers()
    ...
    dsn = (
        f"host='{h._dsn_escape(host)}' port={int(port)} "
        f"dbname='{h._dsn_escape(dbname)}' user='{h._dsn_escape(user)}' "
        f"password='{h._dsn_escape(password)}'"
    )
...
# ---------------------------------------------------------------------------
def writeback_to_parquet(*, duckdb_path, source_sql, output_path,
                          partition_by, compression, dry_run):
    import duckdb
    h = _load_helpers()
    ...
    try:
        ...
        if dry_run:
            print(f"\n[DRY RUN] {count:,} rows -> {safe_path}  "
                  f"compression={safe_comp}  partitions={safe_parts or 'none'}")
            return
        if count > LARGE_WRITE_THRESHOLD:
        ...
        with tempfile.TemporaryDirectory(dir=tp.parent) as tmp:
            ...
            con.execute(
                f"COPY ({source_sql}) TO '{h._sql_str_escape(tmp_out)}' "
...
# ---------------------------------------------------------------------------
def writeback_to_ducklake(*, duckdb_path, source_sql, dl_catalog, dl_data_path,
                           dl_schema, dl_table, dl_alias, conflict_key, dry_run):
    """
    ...
    try:
        ...
        con.execute(
            f"ATTACH 'ducklake:{dl_catalog}' AS {safe_alias} "
            f"(DATA_PATH '{h._sql_str_escape(dl_data_path)}');"
        )
        ...
        if safe_key:
            ...
            sql = (
                f"MERGE INTO {tgt} AS target USING __dl_src AS source "
                f"ON target.{safe_key}=source.{safe_key} "
...
def _parser():
    ...
    pg.add_argument("--pg-host"); pg.add_argument("--pg-port", type=int, default=5432)
    pg.add_argument("--pg-db"); pg.add_argument("--pg-user")
    pg.add_argument("--pg-password"); pg.add_argument("--pg-password-env", metavar="ENV_VAR")
    pg.add_argument("--pg-schema", default="public"); pg.add_argument("--pg-table")
    ...
    dl.add_argument("--dl-catalog"); dl.add_argument("--dl-data-path")
    dl.add_argument("--dl-schema", default="main"); dl.add_argument("--dl-table")
    dl.add_argument("--dl-alias", default="lake"); dl.add_argument("--dl-conflict-key")
    return p
...
def _pg_password(args):
    ...
    print("ERROR: need --pg-password or --pg-password-env", file=sys.stderr); sys.exit(1)
    ...
    if args.target == "postgres":
        for r in ("pg_host","pg_db","pg_user","pg_table"):
            if not getattr(args, r, None):
                print(f"ERROR: --{r.replace('_','-')} required", file=sys.stderr); sys.exit(1)
        writeback_to_postgres(
            duckdb_path=args.source, source_sql=args.query,
            host=args.pg_host, port=args.pg_port,
            dbname=args.pg_db, user=args.pg_user, password=_pg_password(args),
            pg_schema=args.pg_schema, pg_table=args.pg_table,
            conflict_key=getattr(args,"conflict_key",None), dry_run=args.dry_run,
        )
    elif args.target == "parquet":
        if not args.output_path:
            print("ERROR: --output-path required", file=sys.stderr); sys.exit(1)
        writeback_to_parquet(
            duckdb_path=args.source, source_sql=args.query,
            output_path=args.output_path, partition_by=args.partition_by,
            compression=args.compression, dry_run=args.dry_run,
        )
        ...
        writeback_to_ducklake(
            duckdb_path=args.source, source_sql=args.query,
            dl_catalog=args.dl_catalog, dl_data_path=args.dl_data_path,
            dl_schema=args.dl_schema, dl_table=args.dl_table,
            dl_alias=args.dl_alias,
            conflict_key=getattr(args,"dl_conflict_key",None), dry_run=args.dry_run,
        )
